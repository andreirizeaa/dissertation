...
At the same time, this is academic work that others should be able to evaluate and build upon. That means I need to document my decisions, track my progress, and make my process transparent. The structured practices make that possible.
...
\begin{itemize}
    ...
    \item \textbf{Maturity}: Is it well-documented with active community support?
...
\hline
\textbf{Criteria} & \textbf{Next.js} & \textbf{React + TypeScript} & \textbf{Vue.js} & \textbf{Svelte} \\
\hline
...
I chose Next.js because it has become the industry standard for React applications. It includes built-in API routes, which simplifies the architecture by letting me handle simple backend endpoints without a separate server. The framework has strong community support and extensive documentation. TypeScript integration is native, and I can use the full React component library ecosystem.
